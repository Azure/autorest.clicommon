...
import { FlattenSetter } from "./flattensetter";
...
class PropertyInfo {
    ...
    constructor(public property: Property) {
        this.isFlatten = FlattenSetter.isFlattened(property);
        this.isPointToBaseClass = FlattenSetter.isBase(property.schema as ObjectSchema);
    }
    ...
    public unflattenAsCirculeVictim() {
        FlattenSetter.setFlatten(this.property, false);
...
class NodeInfo {
    ...
    public refresh() {
        this.isBaseClass = FlattenSetter.isBase(this.node);
        ...
        if (!isNullOrUndefined(this.node.properties) && this.node.properties.length > 0) {
            for (let i = 0; i < this.node.properties.length; i++) {
                ...
                if (isObjectSchema(p.schema)) {
                    FlattenSetter.isFlattened(p) ? this.flattenProperty.push(new PropertyInfo(p)) : this.unflattenProperty.push(new PropertyInfo(p));
                }
            }
        }
    }
