                ...
                v => `${tab(1)}- operationGroupName: ${generateCliValue(v, 2)}` +
                    `${NEW_LINE}${tab(2)}operations:${NEW_LINE}`.concat(
                        values(v.operations).selectMany(op => [op].concat(NodeHelper.getCliOperation(op, () => []))).select(vv =>
                            ...
                            (isNullOrUndefined(NodeHelper.getExpandOperationOriginalOperation(vv)) ? '' : `${NEW_LINE}${tab(3)}cli-expand-operation-original-operation: ${NodeHelper.getCliKey(NodeHelper.getExpandOperationOriginalOperation(vv), '<missing-clikey>')}`) +
